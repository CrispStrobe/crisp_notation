/// `crisp_notation` command-line tool.
///
/// ```
...
/// (`flutter test tool/render_png.dart` in the `crisp_notation` package), which the
...
const _usage = '''
crisp_notation — music notation CLI

Usage: crisp_notation <command> [arguments]
...
Future<int> _omr(List<String> args) async {
  ...
  final model =
      options['model'] ?? Platform.environment['CRISP_NOTATION_OMR_MODEL'];
  if (model == null || model.isEmpty) {
    throw _CliError(
        'omr needs --model <gguf-or-name> (or CRISP_NOTATION_OMR_MODEL); '
...
/// `render_png.dart` harness in the `crisp_notation` package via `flutter test`
...
int _renderPng(String inPath, String outPath, Map<String, String> options,
  ...
  final pkg = _findCrispNotationDir();
  if (pkg == null) {
    throw _CliError('cannot locate the crisp_notation Flutter package for PNG');
  }
  final env = {
    'CRISP_NOTATION_IN': File(inPath).absolute.path,
    'CRISP_NOTATION_OUT': File(outPath).absolute.path,
    'CRISP_NOTATION_TAB': options.containsKey('tab') ? '1' : '0',
    if (grand) 'CRISP_NOTATION_GRAND': '1',
    if (multiPart) 'CRISP_NOTATION_MULTIPART': '1',
    if (multiPart && options['width'] != null)
      'CRISP_NOTATION_WIDTH': options['width']!,
    if (multiPart && options.containsKey('hide-empty'))
      'CRISP_NOTATION_HIDE_EMPTY': '1',
    if (multiPart && !options.containsKey('no-system-context'))
      'CRISP_NOTATION_SYSTEM_CONTEXT': '1',
    if (multiPart && !options.containsKey('no-title'))
      'CRISP_NOTATION_TITLE': '1',
    if (options['tuning'] != null) 'CRISP_NOTATION_TUNING': options['tuning']!,
    if (options['staff-space'] != null)
      'CRISP_NOTATION_STAFF_SPACE': options['staff-space']!,
  };
...
/// from the running script to the repo's `packages/crisp_notation/assets`.
...
File? _findMetadata(String? override) {
  ...
  for (var i = 0; i < 8; i++) {
    final candidate = File(
        '${dir.path}/packages/crisp_notation/assets/smufl/bravura_metadata.json');
