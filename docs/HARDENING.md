...
* `tool/roundtrip_sweep.dart` — diagnostic over the real corpus at
  ...
  (broken-rhythm sub-beat durations in dense ragtime), not voices. MusicXML /
  MEI / MuseScore preserve every voice. MIDI's lower "exact" is **expected** — the multiset key includes duration, and
