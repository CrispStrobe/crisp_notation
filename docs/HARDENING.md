...
| **Round 16** — K545 engraving parity follow-up + Verovio oracle sweep + round-trip sweep | 8 | Fixed **G24/G25/G27/G28**: MusicXML inline clefs now import/layout at their measure onset; staff-system SVG and Flutter PNG share left-margin instrument labels and original line-start bar numbers; cross-system slurs now render as per-line continuation segments instead of disappearing; omitted initial lower-staff clefs in two-staff MusicXML now default to bass. K545 SVG/PNG now show `Piano`, line starts `5, 8, 11…`, all 101 imported slurs as 104 visible line segments, and lower-staff bass clefs where the current clef is bass. Text markings now use a music-style serif fallback (`Academico`/New York/Times) instead of generic sans where possible. Round-trip stayed **100%** for MusicXML/MEI/kern/ABC/MuseScore over r15. Verovio note oracle is **1/8 exact** and mostly Verovio-only, so log **G26** for the remaining visible-note under-read / oracle-mode investigation rather than treating it as a visual engraving blocker. |
...
| G29 | high (engraving) | **cross-staff slur placement** | K545 still exposes staff-system-aware engraving gaps: long phrase slurs in bars 37-40, 51-53 and 69-70 can occupy the neighboring staff's space because slurs are shaped inside a single-staff layout before the staff system knows about adjacent-staff ink. | K545 SVG/PNG overlap sweep | **open** — partial mitigation: dynamics are laid out before slurs, long slurs get sampled skyline clearance, fake middle-only continuation arcs are skipped, and bass-clef long phrase slurs prefer below. Remaining fix needs explicit slur placement/continuation metadata or a staff-system post-layout collision pass that can move/reshape slurs with knowledge of both staves. |
